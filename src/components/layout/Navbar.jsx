...
export function Navbar() {
  ...
  const isHeroPage = pathname === '/' || pathname.startsWith('/servicios');
  ...
  return (
    <header className={`w-full sticky top-0 z-50 transition-all duration-300 ${
      ...
      {/* 🔴 1. TOP BAR CORPORATIVO (SE OCULTA EN SCROLL DOWN - BLANCA EN MODO OSCURO) */}
      <div 
        className={`w-full text-xs hidden sm:block transition-all duration-300 overflow-hidden ${
          scrolled 
            ? 'h-0 max-h-0 opacity-0 border-b-0 py-0 pointer-events-none' 
            : 'h-10 max-h-12 opacity-100 border-b ' + (
                isTransparent 
                  ? 'bg-slate-950/70 dark:bg-white/90 backdrop-blur-md text-white dark:text-slate-900 border-white/10 dark:border-slate-200' 
                  : 'bg-slate-950 dark:bg-white text-white dark:text-slate-900 border-slate-800/80 dark:border-slate-200'
              )
        }`}
      >
        <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between transition-all duration-300 ${scrolled ? 'h-0 opacity-0' : 'h-10 opacity-100'}`}>
          ...
          <div className="flex items-center gap-2.5">
            <a 
              ...
              className="p-1.5 rounded-lg bg-white/20 dark:bg-slate-100 hover:bg-[#e11b22] dark:hover:bg-[#e11b22] text-white dark:text-slate-900 hover:text-white dark:hover:text-white transition-all flex items-center justify-center shadow-xs active:scale-95"
            >
              <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24"><path fill="currentColor" d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            </a>
            <a 
            ...
            <a 
              ...
              className="p-1.5 rounded-lg bg-white/20 dark:bg-slate-100 hover:bg-[#e11b22] dark:hover:bg-[#e11b22] text-white dark:text-slate-900 hover:text-white dark:hover:text-white transition-all flex items-center justify-center shadow-xs active:scale-95"
            >
              <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
            </a>
          </div>
          ...
          <div className="flex items-center gap-6 font-medium">
            <div className="flex items-center gap-2">
              <Mail className="w-3.5 h-3.5 text-[#e11b22] shrink-0" />
              <span className="font-semibold"><span>info</span><span className="text-[#e11b22]">&#64;</span><span>dcasesoresec.com</span></span>
            </div>
            <div className="flex items-center gap-2">
              <Phone className="w-3.5 h-3.5 text-[#e11b22] shrink-0" />
              <a href="tel:025003373" className="hover:text-[#e11b22] transition-colors font-bold">
                02 500 3373
              </a>
            </div>
            <Link 
      ...
      {/* 🔴 2. NAVBAR PRINCIPAL DUAL (TRANSPARENTE EN INICIO / SOLID BLUR AL SCROLL) */}
      <div className={`w-full transition-all duration-300 ${
        isTransparent
          ? 'bg-transparent border-b border-transparent shadow-none'
          : 'bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl border-b border-slate-200/80 dark:border-slate-800 shadow-md'
      }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-20">
          
          {/* Logo dual adaptativo */}
          <Link href="/" onClick={closeMobileMenu} className="flex items-center gap-3 group">
            {/* Si es transparente al inicio, mostramos el logo blanco que resalta sobre la foto del Hero */}
            {isTransparent ? (
              <img 
              ...
              <>
                <img 
                  src="/logo-dark-text.webp" 
                  alt="DC Asesores en Seguros" 
                  className="h-10 md:h-12 w-auto object-contain transition-transform group-hover:scale-105 block dark:hidden" 
                />
                <img 
                  src="/logo-white-text.webp" 
                  alt="DC Asesores en Seguros" 
                  className="h-10 md:h-12 w-auto object-contain transition-transform group-hover:scale-105 hidden dark:block" 
                />
              </>
            )}
            
            <div className={`flex flex-col border-l-2 pl-3 ${
              isTransparent ? 'border-white/30' : 'border-slate-200 dark:border-slate-800'
            }`}>
              <span className={`text-base md:text-lg font-black tracking-tight leading-none ${
          ...
          <nav className="hidden lg:flex gap-6 xl:gap-8 items-center font-bold text-xs uppercase tracking-wider">
            {navLinks.map((link) => {
            ...
            <Link 
              ...
              className={`px-5 py-2.5 rounded-full font-black text-xs uppercase tracking-wider transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5 active:scale-95 flex items-center gap-2 ${
                pathname === '/cotizador'
                  ...
                  : isTransparent
                    ? 'bg-[#e11b22] text-white hover:bg-red-600 shadow-red-600/30'
          ...
          <div className="pt-2 flex flex-col gap-3">
            ...
            <div className="pt-2 flex items-center justify-center gap-4">
              <a 
                ...
                rel="noreferrer" 
                aria-label="Facebook DC Asesores"
                className="p-2.5 rounded-xl bg-slate-900 dark:bg-slate-800 text-white hover:bg-[#e11b22] dark:hover:bg-[#e11b22] hover:text-white dark:hover:text-white transition-all shadow-xs active:scale-95"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24"><path fill="currentColor" d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
              <a 
                href="https://www.tiktok.com/@asesorseguros2?_r=1&_t=ZS-94CCdpAiPGZ" 
                target="_blank" 
                ...
                className="p-2.5 rounded-xl bg-slate-900 dark:bg-slate-800 text-white hover:bg-[#e11b22] dark:hover:bg-[#e11b22] hover:text-white dark:hover:text-white transition-all shadow-xs active:scale-95"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24"><path fill="currentColor" d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.24 1.07-.14 1.61.24 1.64 1.82 2.89 3.5 2.64 1.25-.11 2.37-.93 2.78-2.11.23-.6.3-1.25.28-1.89.02-4.97.01-9.94.01-14.91z"/></svg>
              </a>
              <a 
                href="https://www.instagram.com/dc.asesorseguros?igsh=MXMwd2oxbWkza3hjdw%3D%3D" 
                target="_blank" 
                ...
                className="p-2.5 rounded-xl bg-slate-900 dark:bg-slate-800 text-white hover:bg-[#e11b22] dark:hover:bg-[#e11b22] hover:text-white dark:hover:text-white transition-all shadow-xs active:scale-95"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
              </a>
            </div>
          </div>
        </div>
