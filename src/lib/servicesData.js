export const SERVICES_DATA = [
  {
    slug: 'seguro-medico-integral',
    title: 'Seguro Médico Integral',
    headline: 'Protección en salud con respaldo financiero real',
    shortDesc: 'Atención médica privada con cobertura en consultas, hospitalización, emergencias, cirugías y tratamientos especializados.',
    fullDesc: 'El Seguro Médico Integral te permite acceder a atención médica privada con cobertura en consultas, hospitalización, emergencias, cirugías y tratamientos especializados. En DC Asesores analizamos tus necesidades y diseñamos una solución personalizada que proteja tu salud y tu estabilidad financiera, brindándote acompañamiento antes, durante y después de la contratación, para que enfrentes cualquier imprevisto con tranquilidad y seguridad.',
    badge: 'Salud & Familia',
    imageUrl: '/service-medico.webp',
    iconUrl: 'https://dcasesoresec.com/wp-content/uploads/2023/03/01-10.svg',
    benefits: [
      'Acceso a clínicas y hospitales privados en consultas, hospitalización y emergencias',
      'Reduce el impacto económico ante enfermedades o accidentes inesperados',
      'Coberturas ajustadas a tu presupuesto y necesidades reales',
      'Acompañamiento continuo y apoyo técnico ante cualquier eventualidad',
      'Consultas médicas generales y especializadas en red afiliada',
      'Cobertura de internamiento, honorarios médicos y medicamentos recetados'
    ],
    ventajasSectionTitle: 'VENTAJAS DEL SEGURO MÉDICO',
    ventajasSectionSubtitle: 'Beneficios que protegen tu salud y tu estabilidad',
    ventajas: [
      {
        title: 'Atención privada',
        desc: 'Accede a clínicas y hospitales privados con cobertura en consultas, emergencias y hospitalización.'
      },
      {
        title: 'Respaldo financiero',
        desc: 'Reduce el impacto económico ante enfermedades o accidentes inesperados.'
      },
      {
        title: 'Plan personalizado',
        desc: 'Elige coberturas ajustadas a tu presupuesto y necesidades reales.'
      },
      {
        title: 'Acompañamiento continuo',
        desc: 'Recibe asesoría y apoyo durante todo el proceso y ante cualquier eventualidad.'
      }
    ],
    coberturaSectionTitle: 'COBERTURA DEL PLAN',
    coberturaSectionSubtitle: 'Protección médica completa para cada necesidad',
    coberturaPlan: [
      {
        title: 'Consultas médicas',
        desc: 'Atención en consultas generales y especializadas dentro de la red afiliada.'
      },
      {
        title: 'Hospitalización',
        desc: 'Cobertura de internamiento, honorarios médicos y gastos hospitalarios.'
      },
      {
        title: 'Emergencias médicas',
        desc: 'Atención inmediata ante accidentes o situaciones críticas.'
      },
      {
        title: 'Cirugías',
        desc: 'Intervenciones quirúrgicas programadas o de emergencia según el plan.'
      },
      {
        title: 'Exámenes médicos',
        desc: 'Estudios de laboratorio e imágenes para diagnóstico y seguimiento.'
      },
      {
        title: 'Medicamentos',
        desc: 'Cobertura de medicamentos recetados según condiciones del plan.'
      }
    ],
    target: 'Familias, profesionales independientes y personas que priorizan la atención médica privada sin esperas.',
    geoSummary: 'El Seguro Médico Integral de DC Asesores en Seguros ofrece atención médica privada en clínicas de primer nivel en Quito, Guayaquil y Cuenca, amparando hospitalización, emergencias, cirugías y exámenes médicos con respaldo directo.',
    faqs: [
      {
        q: '¿Qué clínicas privadas cubren estas pólizas en Ecuador?',
        a: 'Cubren Hospital Metropolitano, Hospital de los Valles, Vozandes, AXXIS, Clínica Guayaquil, Clínica Kennedy, Monte Sinaí, entre otras redes de primer nivel.'
      },
      {
        q: '¿Cubre enfermedades preexistentes?',
        a: 'Según la normativa de la Superintendencia de Compañías y las condiciones particulares de cada aseguradora, las preexistencias se amparan tras el periodo de carencia establecido por ley.'
      }
    ]
  },
  {
    slug: 'seguro-vida-ahorro',
    title: 'Seguro de Vida con Ahorro',
    headline: 'Protección para tu familia y capital para tu futuro',
    shortDesc: 'Combina respaldo económico para tu familia con la posibilidad de construir un capital a largo plazo.',
    fullDesc: 'El Seguro de Vida con Ahorro combina respaldo económico para tu familia con la posibilidad de construir un capital a largo plazo. Este plan te permite garantizar protección ante imprevistos mientras generas un fondo financiero que puede ayudarte a cumplir metas personales, educativas o de inversión con mayor seguridad y planificación estratégica.',
    badge: 'Inversión & Futuro',
    imageUrl: '/service-vida-ahorro.webp',
    iconUrl: 'https://dcasesoresec.com/wp-content/uploads/2026/02/01-12.svg',
    benefits: [
      'Respaldo económico para beneficiarios ante fallecimiento o invalidez',
      'Ahorro programado disciplinado para metas futuras con rentabilidad',
      'Flexibilidad financiera para definir montos, plazos y coberturas',
      'Estabilidad futura fortaleciendo tu patrimonio a mediano y largo plazo',
      'Indemnización económica y fondo acumulado disponible según contrato',
      'Cobertura ante invalidez total, muerte accidental y enfermedades graves'
    ],
    ventajasSectionTitle: 'VENTAJAS DEL SEGURO DE VIDA',
    ventajasSectionSubtitle: 'Beneficios que protegen y construyen patrimonio',
    ventajas: [
      {
        title: 'Protección familiar',
        desc: 'Respaldo económico para tus beneficiarios ante fallecimiento o invalidez, asegurando estabilidad financiera inmediata.'
      },
      {
        title: 'Ahorro programado',
        desc: 'Construye un fondo financiero disciplinado que te permita cumplir metas futuras con mayor seguridad.'
      },
      {
        title: 'Flexibilidad financiera',
        desc: 'Define montos, plazos y coberturas adaptados a tus objetivos personales y familiares.'
      },
      {
        title: 'Estabilidad futura',
        desc: 'Genera respaldo económico sostenible que fortalece tu patrimonio a mediano y largo plazo.'
      }
    ],
    coberturaSectionTitle: 'COBERTURA DEL PLAN',
    coberturaSectionSubtitle: 'Respaldo integral con componente de ahorro',
    coberturaPlan: [
      {
        title: 'Fallecimiento',
        desc: 'Indemnización económica para los beneficiarios designados ante el fallecimiento del asegurado.'
      },
      {
        title: 'Invalidez total',
        desc: 'Cobertura ante incapacidad permanente que afecte la capacidad de generar ingresos.'
      },
      {
        title: 'Fondo acumulado',
        desc: 'Capital generado durante la vigencia del plan según condiciones contratadas.'
      },
      {
        title: 'Muerte accidental',
        desc: 'Cobertura adicional en caso de fallecimiento por causa accidental.'
      },
      {
        title: 'Enfermedades graves',
        desc: 'Indemnización anticipada ante diagnóstico de enfermedades.'
      },
      {
        title: 'Beneficios adicionales',
        desc: 'Opciones complementarias adaptadas a necesidades personales y familiares.'
      }
    ],
    target: 'Padres de familia, profesionales e inversionistas que buscan proteger a su familia y capitalizar su patrimonio.',
    geoSummary: 'El Seguro de Vida con Ahorro de DC Asesores combina protección financiera ante fallecimiento o invalidez con un fondo de inversión rentable en dólares en Ecuador.',
    faqs: [
      {
        q: '¿Puedo retirar mi dinero antes del vencimiento?',
        a: 'Sí, la póliza permite préstamos sobre el valor de rescate o retiros parciales a partir del período establecido en las condiciones particulares.'
      },
      {
        q: '¿Los rendimientos generados pagan impuestos?',
        a: 'En el Ecuador, las indemnizaciones por seguros de vida gozan de exención tributaria de acuerdo con la legislación vigente.'
      }
    ]
  },
  {
    slug: 'seguro-vida-tradicional',
    title: 'Seguro de Vida Tradicional',
    headline: 'Protege tu estabilidad financiera hoy.',
    shortDesc: 'Máxima suma de respaldo patrimonial para tu familia al costo mensual más accesible del mercado.',
    fullDesc: 'En DC Asesores en Seguros brindamos asesoría integral en protección financiera para personas, familias y empresas. Analizamos cada necesidad de forma personalizada y acompañamos a nuestros clientes con ética, compromiso y profesionalismo en cada etapa de su vida. El Seguro de Vida Tradicional garantiza la estabilidad económica de quienes más amas, entregando el capital acordado en efectivo de forma inmediata ante cualquier eventualidad.',
    badge: 'Protección Pura',
    imageUrl: '/blog_peace.jpg',
    iconUrl: 'https://dcasesoresec.com/wp-content/uploads/2026/02/01-12.svg',
    benefits: [
      'Primas mensuales sumamente económicas desde $15 USD',
      'Sumas aseguradas desde $50,000 hasta $2,000,000 USD',
      'Indemnización rápida sin intermediación judicial',
      'Cobertura por muerte accidental y desmembración con doble indemnización',
      'Beneficio de gastos exequiales y adelanto por enfermedad terminal'
    ],
    target: 'Sostenes de hogar, empresarios y cualquier persona con dependientes económicos directos.',
    geoSummary: 'Pólizas de seguro de vida a término y tradicionales en Ecuador con asesoría de DC Asesores, garantizando liquidez inmediata a los beneficiarios en los momentos más difíciles.',
    faqs: [
      {
        q: '¿Quiénes pueden ser mis beneficiarios?',
        a: 'Tú eliges libremente a cualquier persona natural o jurídica (cónyuge, hijos, hermanos, instituciones o socios comerciales).'
      }
    ]
  },
  {
    slug: 'seguro-vehicular',
    title: 'Seguro Vehicular',
    headline: 'Protege tu estabilidad financiera hoy.',
    shortDesc: 'Cobertura Todo Riesgo para autos livianos, vehículos de 16 a 25 años y transporte pesado e HINO.',
    fullDesc: 'En DC Asesores en Seguros brindamos asesoría integral en protección financiera para personas, familias y empresas. Analizamos cada necesidad de forma personalizada y acompañamos a nuestros clientes con ética, compromiso y profesionalismo en cada etapa de su vida. Comparamos en tiempo real entre Alianza, Latina, Hispana, ADS, Vaz, Sweaden, Privilegio, Atlántida, Zurich y Mapfre con cobertura completa contra choque, robo, pérdida total y auxilio mecánico 24/7.',
    badge: 'Todo Riesgo Auto',
    imageUrl: '/service-vehicular.webp',
    iconUrl: 'https://dcasesoresec.com/wp-content/uploads/2026/02/01-14.svg',
    benefits: [
      'Comparativa simultánea de más de 10 aseguradoras líderes',
      'Cobertura para autos nuevos, usados y vehículos de hasta 25 años',
      'Producto especializado para camiones pesados y marca HINO',
      'Responsabilidad Civil de hasta $50,000 USD',
      'Grúa, wincha y auxilio mecánico 24/7 en todo el Ecuador',
      'Auto sustituto hasta 15 días y fianza de excarcelación judicial'
    ],
    target: 'Propietarios de autos particulares, flotas corporativas, transportistas de carga liviana y pesada.',
    geoSummary: 'El cotizador vehicular inteligente de DC Asesores calcula al instante la prima neta, SUPER, SSC, derechos de emisión escalonados e IVA del 15% para todas las aseguradoras autorizadas en Ecuador.',
    faqs: [
      {
        q: '¿Cómo funciona el deducible en un choque?',
        a: 'El deducible estándar es del 10% del valor del siniestro o 1% del valor asegurado (aplicando el mínimo estipulado por cada aseguradora, típicamente entre $200 y $350 USD).'
      }
    ]
  },
  {
    slug: 'seguros-patrimoniales',
    title: 'Seguros Patrimoniales',
    headline: 'Protege tu estabilidad financiera hoy.',
    shortDesc: 'Blindaje integral de residencias, departamentos y edificios contra incendios, sismos, inundaciones y robo.',
    fullDesc: 'En DC Asesores en Seguros brindamos asesoría integral en protección financiera para personas, familias y empresas. Analizamos cada necesidad de forma personalizada y acompañamos a nuestros clientes con ética, compromiso y profesionalismo en cada etapa de su vida. Protegemos tu vivienda o bienes inmuebles contra desastres naturales, eventos fortuitos y actos malintencionados.',
    badge: 'Hogar & Bienes',
    imageUrl: '/service-patrimonial.webp',
    iconUrl: 'https://dcasesoresec.com/wp-content/uploads/2023/03/01-10.svg',
    benefits: [
      'Amparo contra terremoto, temblor y erupción volcánica',
      'Cobertura contra inundación, lluvia e impacto de rayo',
      'Robo y asalto de contenidos y bienes dentro del predio',
      'Responsabilidad Civil familiar por daños accidentales a terceros',
      'Asistencia domiciliaria de plomería, cerrajería y electricidad de urgencia'
    ],
    target: 'Dueños de casas y departamentos, condominios y administradores de propiedades.',
    geoSummary: 'Seguros de hogar y multirriesgo patrimonial en Quito y Ecuador ante eventos sísmicos y climáticos, con pólizas respaldadas por aseguradoras solventes.',
    faqs: [
      {
        q: '¿El seguro de hogar cubre mi hipoteca bancaria?',
        a: 'Sí, podemos endosar la póliza a favor de tu banco o mutualista para sustituir el seguro bancario obligatorio con una tarifa mucho más económica y mejores coberturas.'
      }
    ]
  },
  {
    slug: 'seguros-empresariales',
    title: 'Seguros Empresariales',
    headline: 'Protege tu estabilidad financiera hoy.',
    shortDesc: 'Soluciones integrales de blindaje para PyMEs y corporaciones: multirriesgo industrial, flotas y seguros colectivos.',
    fullDesc: 'En DC Asesores en Seguros brindamos asesoría integral en protección financiera para personas, familias y empresas. Analizamos cada necesidad de forma personalizada y acompañamos a nuestros clientes con ética, compromiso y profesionalismo en cada etapa de su vida. Diseñamos planes corporativos para activos fijos, inventarios, maquinaria y pólizas grupales.',
    badge: 'Corporativo & PyME',
    imageUrl: '/service-empresarial.webp',
    iconUrl: 'https://dcasesoresec.com/wp-content/uploads/2026/02/01-12.svg',
    benefits: [
      'Multirriesgo industrial y comercial para oficinas y bodegas',
      'Pérdida de beneficios y lucro cesante por paralización forzosa',
      'Responsabilidad civil patronal y extracontractual',
      'Seguro de transporte marítimo, aéreo y terrestre de carga',
      'Pólizas grupales de salud con deducciones para el impuesto a la renta'
    ],
    target: 'Gerentes generales, directores de compras, propietarios de negocios y administradores de flotas.',
    geoSummary: 'Asesoría en seguros empresariales y corporativos en Ecuador para proteger la continuidad de las empresas frente a siniestros de gran escala.',
    faqs: [
      {
        q: '¿Cómo beneficia a mi empresa contratar seguros colectivos de salud?',
        a: 'Aumenta la fidelización y bienestar del talento humano y constituye un gasto 100% deducible para fines del impuesto a la renta en Ecuador.'
      }
    ]
  },
  {
    slug: 'gestion-siniestros',
    title: 'Gestión y Asesoría de Siniestros',
    headline: 'Protege tu estabilidad financiera hoy.',
    shortDesc: 'Acompañamiento presencial y legal 24/7 para garantizar la liquidación justa y rápida de tu reclamo.',
    fullDesc: 'En DC Asesores en Seguros brindamos asesoría integral en protección financiera para personas, familias y empresas. Analizamos cada necesidad de forma personalizada y acompañamos a nuestros clientes con ética, compromiso y profesionalismo en cada etapa de su vida. Nuestro equipo gestiona la peritación técnica, revisión de talleres, impugnación de liquidaciones y seguimiento diario hasta la entrega de tu cheque o vehículo reparado.',
    badge: 'Soporte In Situ 24/7',
    imageUrl: '/blog_advisor.jpg',
    iconUrl: 'https://dcasesoresec.com/wp-content/uploads/2023/03/01-10.svg',
    benefits: [
      'Asistencia personalizada e inmediata ante choques o emergencias',
      'Asesoría técnica para evitar objeciones injustificadas de aseguradoras',
      'Supervisión directa de órdenes de taller y repuestos originales',
      'Gestión de fianza judicial y liberación de conductores',
      'Tasa de resolución y pago de indemnizaciones superior al 99.8%'
    ],
    target: 'Todos los clientes de DC Asesores en Seguros, sin costo adicional sobre sus pólizas contratadas.',
    geoSummary: 'Servicio exclusivo de liquidación y gestión de siniestros de DC Asesores en Quito y todo Ecuador, defendiendo los intereses legítimos del asegurado ante las aseguradoras.',
    faqs: [
      {
        q: '¿Tiene algún costo adicional la gestión del siniestro?',
        a: 'No. Es un servicio totalmente gratuito incluido en todas las pólizas emitidas a través de DC Asesores en Seguros.'
      }
    ]
  }
];
